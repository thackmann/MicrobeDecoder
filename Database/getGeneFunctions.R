# Get Gene Families
# This script gets gene functions (KO IDs) for organisms in the database
...
# === Get directories ===
  ...
  output_directory <- file.path(root_directory, "Shiny/MicrobeDecoder/data/gene_functions")
  # genome_directory <- file.path("C:\\My Directory\\img_cart") # Set to directory where files above were downloaded
...
#=== Process genome files ===
  ...
  # Loop over genome folders
    for (i in seq_along(genome_folders)) {
      ...
      data <- read_annotation_tab_file (genome_folders[i], database = "ko")
  ...
  # Reduce size of dataframe
    KO_data_compressed <- KO_data |>
      ...
      dplyr::filter(!is.na(Database_ID), Database_ID != "") |> # Remove rows with no values
...
#===  Export ===
  ...
  saveRDS(KO_data_compressed, file = "gene_functions_database.rds")
