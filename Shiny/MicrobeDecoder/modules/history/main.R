...
historyServer <- function(input, output, session, x, selected_tab, show_all_users = FALSE) {
  ...
  get_table <- eventReactive(combined_trigger(), {
    ...
    if (show_all_users) {
      user_dirs <- list.dirs("jobs", recursive = FALSE, full.names = TRUE)
    } else {
    ...
    tool_dirs <- list.dirs(user_dirs, recursive = TRUE, full.names = TRUE)
    ...
    users <- basename(dirname(subdirs))                            # Get user folders (user IDs)
    ...
    tab_query <- paste0("?tab=", tabs, "&user=", users, "&job=", job_ids)
    urls <- paste0(
