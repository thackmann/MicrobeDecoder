...
#'
#' @param query_string A query string used to filter the data.
...
#' @export
compute_search_results <- function(query_string, data, col_name, ns = NULL) {
  # Update progress
  if (!is.null(ns)) display_modal(ns = ns, message = "Performing search")
...
# === Get inputs ===
  ...
  #'
  #' @return A named list containing `query_string`.
  get_search_inputs <- function(query_string) {
    list(
