...
# === Functions for solving metabolic networks ===
  ...
  #'
  #' @param gene_functions A dataframe with gene functions for each organism (columns = organisms).
  #' @param organism_names A character vector of organism names (must match columns of gene_functions).
  #' @param reference_network_model A dataframe representing the reference metabolic network.
  ...
  #' @param ns A namespace function (optional; used for progress bar IDs in Shiny modules).
  #'
  ...
  get_solved_models <- function(gene_functions, organism_names,
                                         ...
                                         ns = NULL) {
    solved_model <- setNames(vector("list", length(organism_names)), organism_names)
    for (i in seq_along(organism_names)) {
      ...
      solved_model[[organism_name]] <- solve_network_model(
      ...
      # Update progress
      if (!is.null(ns)) display_modal(ns = ns, message = "Prediction in progress", value = (i / length(organism_names)) * 100)
    }
  ...
  #'
  #' @param solved_models A nested list of solved network models from `get_solved_models()`.
  #'   Structure: [organism][substrate][product] = dataframe with flux results.
...
# === Loading and processing configuration file ===  
  ...
  #' If the reference is not found, the fallback `"Other"` entry is returned.
  #'
  #' @param tab A character string indicating the selected tab (e.g., `"Database"` or `"File upload"`).
  #' @param database_val A character string specifying the selected reference reaction from the database tab.
  #'
  #' @return A named list (or row) corresponding to the configuration for the selected reference reaction.
  #' @export
  #'
  #' @examples
  ...
  get_network_plot_config <- function(network_dimensions) {
    config <- load_config_networks_plots()
  ...
  #'   }
  #' @export
  #' @seealso \code{\link{get_network_plot_config}}, \code{\link{load_config_networks_plots}}
