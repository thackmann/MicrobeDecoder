...
clean_external_data = function(x, is_numeric = FALSE, to_lower = FALSE) {
  if(is_numeric == TRUE) {
    ...
    # Remove alpha-, beta-, D-, L-, (+), and (-) in chemical names
    x <- gsub("\\(\\+\\)-", "", x)  # remove (+)-
    x <- gsub("\\(-\\)-", "", x)   # remove (-)-
    x <- gsub("\\(\\+\\)", "", x)  # remove (+)
    x <- gsub("\\(-\\)", "", x)    # remove (-)
    ...
    x <- gsub("\\(R\\)", "", x)    # remove (R)
    x <- gsub("\\(S\\)", "", x)    # remove (S)
    x <- gsub("\\b(alpha|beta|d|D|l|L)-", "", x)  # remove alpha-, beta-, D-, L- (case-insensitive)
    
    # Move numbers out of parentheses if they're followed by a dash
    x <- gsub("\\((\\d+)\\)-", "\\1-", x)
...
create_query_filter <- function(id, data, type = "string", delimited = FALSE, delimiter = ";", label = NULL) {
  label <- label %||% id  # use id if label is not provided
  
  if (type == "string") {
    plugin_config <- create_selectize_plugin_config(
      vec = data[[id]],
      delimited = delimited,
      delimiter = delimiter
    )
