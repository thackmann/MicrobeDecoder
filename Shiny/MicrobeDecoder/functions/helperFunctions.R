...
# === Navigation of tabs ===
  ...
  #' Navigate to Tab Based on Query in URL
  #'
  #' Navigates to the correct tab when the Shiny app first loads.  It parses the
  #' the tab based on the `?tab=` query parameter in the url.  After navigation, 
  #' it hides the loading screen and updates navigation counters.  
  #' It sends messages to track if computation job is available or if the
  #' app is being launched by IMG.
  #' 
  ...
  #' @export
  get_tab_from_query <- function(session = getDefaultReactiveDomain(),
    ...
    observe({
      if (is.null(session$userData$has_initialized) || !session$userData$has_initialized) {
        ...
        # Get tab, job, and other other query parameters
        tab <- get_query_param(param_name = "tab")
        job <- get_query_param(param_name = "job")

        # Send message if a computation job is available
        session$userData$loaded_job_on_init <- !is.null(job) && nzchar(job)
        session$sendCustomMessage("loaded_job_on_init", session$userData$loaded_job_on_init)
  ...
  #' It also clears the `?job=` and `?user=` parameters after the user changes tabs manually
  #'
  #' @param session The Shiny session object.
  ...
  #' @export
  get_query_from_tab <- function(session = getDefaultReactiveDomain(),
                                           input = getDefaultReactiveDomain()$input,
                                           tab_input_id = "tabs") {
    observeEvent(input[[tab_input_id]], {
      ...
      # Clear the `?job=` and `?user=` parameters after the user changes tabs manually
  ...
  #' - **Navigating to the correct tab when the app loads** based on the query string (e.g., `?tab=results`)
  ...
  sync_tabs_with_query <- function(session = getDefaultReactiveDomain(),
                                   input = getDefaultReactiveDomain()$input,
                                   tab_input_id = "tabs") {
    # Load tab from query when app initializes
    get_tab_from_query(session, input, tab_input_id)
...
# === Job submission and retrieval ===  
  ...
  #' Construct a URL for a job based on tab name, job ID, and user ID
  #'
  #' @param session The Shiny session object
  ...
  #'
  #' @param input The Shiny input object. Defaults to the current reactive domain.
  ...
  #' Check if a job result file exists
  #'
  #' This function checks whether a `.rds` file for a given job ID exists in the specified directory.
  #'
  #' @param job_id Job ID string (e.g., "abc123")
  #' @param job_dir Path to the base job directory. Defaults to "jobs".
  #' @return TRUE if the job file exists, FALSE otherwise
  #' @export
  job_exists <- function(job_id, job_dir = "jobs") {
    # Construct full path to job file
    job_file <- file.path(job_dir, paste0(job_id, ".rds"))
    
    # Check if the file exists
    file.exists(job_file)
  }
...
# === Clear old jobs ===    
  ...
  #'
  #' Periodically deletes old or large files from a given directory.
  #'
  #' @inheritParams cleanup_old_large_files
  #' @param session The Shiny session object (used by `reactiveTimer`). Defaults to `getDefaultReactiveDomain()`.
  ...
  setup_auto_cleanup <- function(session = getDefaultReactiveDomain(),
    ...
    observe({
      ...
      t1 <- Sys.time()
      deleted <- cleanup_old_large_files(
        dir = dir,
...
# === Modals ===
  ...
  #' @param session The Shiny session object.
	...
	#'
	#' @param session The Shiny session object.
	#' @param ns A namespacing function (default = identity). Used to namespace the `id`.
	...
	#' @param delay_time The delay in milliseconds before hiding the modal (default: 1000).
	#'
...
# === File reading ===
	...
	#' @param accepted_extensions Character vector. The accepted file extensions (default: c("csv", "tsv", "txt", "rds", "ko", "xls", "xlsx", "zip")).
	#' @param detect_comments Logical.  Detect comment lines in file?
